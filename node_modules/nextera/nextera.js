...
dirRoot = "/home/concussed/concussionjs-testdrive/";
...
var dbSL = new DbSL('concussion-prod', new serverSL(host, port, {}), {native_parser: false});
var db;
...
module.exports.getEntriesByTenantObjectId = function(whereRegExp,collectionName,callback)
  ...
  var reg = new RegExp('.*' + escapeSpecialCharacters(whereRegExp).split(' ').join('.*|.*') + '.*', 'i');
  db.collection(collectionName, function(err, collection) {
    collection.find({'tenant_object_id': reg}).toArray(function(err, documents) {
      if (err)
      {
        console.error(err);
      }
...
module.exports.serveStaticFilesNoWriteHead = function(req,res,next) {
  ...
  path.exists(filename, function(exists) {
    ...
    fs.readFile(filename, 'binary', function(err, file) 
          ...
          if ((filename.search('admin') > -1 || filename.search('.html') > -1 || filename.search('/index') > -1) && uri.search('.js') == -1 && filename.search('.css') == -1)
				{
