...
var html = fs.readFileSync("kotemplate-scriptonly.ejs","utf-8");
var adminhtml = fs.readFileSync("kotemplate-admin.ejs","utf-8");
var superadminhtml = fs.readFileSync("kotemplate-superadmin.ejs","utf-8");
var ejs = require("ejs");
...
module.exports.getEntriesByName=function(whereRegExp,collectionName,callback)
{
        ...
        db.collection(collectionName, function(err, collection) {
                collection.find({"name":reg}).toArray(function(err, documents){
                        if(err)
                        {
                                console.log("err:",err);                
                        }
	...
	if(req.url.search("concussionjs.com")>-1)
		...
		path.exists(filename, function(exists) {               	
            ...
            fs.readFile(filename, "binary", function(err, file) 
			{
				...
				console.log("about to write head, filename: ",filename, " ", filename.search("admin") );
			 
                    if(filename.search("svg")>-1)
                        res.writeHead(200, {"Content-Type": "image/svg+xml"});
                    else if(filename.search("png")>-1)
                        res.writeHead(200, {"Content-Type": "image/png"});  
                    else
                        res.writeHead(200,{'Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'X-Requested-With'});    
    
                //console.log("about ",filename," " ,(filename.search("admin")>-1 || filename.search(".html")>-1)," .js " , uri.search(".js"), " ",filename.search(".js")== -1 ," .css", filename.search(".css")== -1);
                if((filename.search("admin")>-1 || filename.search(".html")>-1) && uri.search(".js")== -1 && filename.search(".css")== -1)
				{
											
			  		console.log("about getPage: nextera.js");
